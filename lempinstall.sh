...
Set up NGINX Stable repositories
echo "setting up the apt repository for stable nginx packages"

echo "deb http://nginx.org/packages/mainline/ubuntu/ `lsb_release -cs` nginx" | sudo tee -a /etc/apt/sources.list
echo "deb-src http://nginx.org/packages/mainline/ubuntu/ `lsb_release -cs` nginx" | sudo tee -a /etc/apt/sources.list
...
# compile Nginx from the official repo with brotli and pagespeed compression
...
sudo apt-get -qq install dpkg-dev  git  uuid-dev build-essential zlib1g-dev libpcre3 libpcre3-dev unzip fail2ban ufw curl gnupg2 ca-certificates lsb-release
echo done.
...
apt-get install -y build-essential tree curl nano wget lftp unzip bzip2 arj nomarch lzop htop openssl gcc git binutils libmcrypt4 libpcre3-dev make python3 python3-pip supervisor unattended-upgrades whois zsh imagemagick uuid-runtime net-tools
...
sudo ln -s /usr/local/src/${USER}/ngx_pagespeed /usr/local/src/ngx_pagespeed
sudo chown ${USER}:$USER:$(id -gn $USER) /usr/local/src/ngx_pagespeed
cd /usr/local/src/${USER}/nginx-*/

# modify the existing config
sed -i -e '/\.\/configure/ s:$: --add-module=/usr/local/src/ngx_pagespeed:' debian/rules
...
# To clean up after install You can run
 ...
 sudo rm /etc/apt/sources.list.d/nginx.list.d/nginx.list
 sudo apt-get -qq update
"
...
echo "nginx has been installed with the following modules."
nginx -V 2>&1 | tr -- - '\n' | grep module
